/**
 * A panasz-névsor értékkészletének előállítása a panaszszótárból.
 *
 * A `compl.active` és a `compl.chief` értékkészlete NEM kézi lista: a szótár
 * (`registry/complaints/`) a forrás. Ez a generátor írja be a regiszterfájlba,
...
const FILE = "registry/variables/01-panaszok.json";
const ROSTERS = ["compl.active", "compl.chief"];
...
if (process.argv.includes("--check")) {
  if (changed) {
    console.error(
      `✗ ${FILE}: a névsor nem egyezik a panaszszótárral (${want.length} tétel).\n` +
      `  Futtasd: node tools/gen-complaint-roster.ts`,
    );
    process.exit(1);
  }
  console.log(`✓ panasz-névsor naprakész (${want.length} tétel)`);
} else {
